...
MSYS := $(if $(or $(findstring cygwin,$(OSTYPE)),$(findstring msys,$(OSTYPE))),1)
WSL := $(if $(WSL_DISTRO_NAME),1)
...
FONTS := $(if $(or $(WSL),$(MSYS)),,.fonts_installed)
BRANCH := master
...
PYMS := powerline $(if $(MSYS),,psutil) pylint pip_search
